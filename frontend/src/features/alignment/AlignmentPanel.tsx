...
function AlignmentDetail({ alignmentId }: { alignmentId: string }) {
  ...
  const curveCount = alignment.elements.filter((e) => e.type === "curve").length;
        ...
        <div className="rounded bg-slate-100 px-2 py-1.5 text-[10px] dark:bg-slate-800">
          ...
          {alignment.ipPoints.map((p, i) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => store.removeIpPoint(alignmentId, p.id)}
      ...
      {alignment.elements.length > 0 && (
        <div className="rounded bg-slate-100 px-2 py-2 text-[11px] dark:bg-slate-800">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">エレメント数</span>
              <span className="font-mono font-semibold text-slate-700 dark:text-slate-200">
...
export default function AlignmentPanel() {
  ...
  function handleCreate() {
    ...
    store.addAlignment(name, newSpeed);
    setNewName("");
  }
