...
// ---- ProjectPanel: delete project ------------------------------------------

test("ProjectPanel: プロジェクト未選択時に削除ボタンが表示されない", async ({
  page,
}) => {
  await setupApiMocks(page);
  await page.goto("/");
  ...
  await page.getByRole("button", { name: "ログイン" }).last().click();
  await expect(page.getByRole("button", { name: "ログアウト" })).toBeVisible();
  // Do NOT select a project — delete button should be hidden
  await expect(
    page.getByRole("button", { name: "プロジェクトを削除" }),
  ).not.toBeVisible();
});
...
test("ProjectPanel: プロジェクト削除確認ダイアログキャンセルでAPIが呼ばれない", async ({
  page,
}) => {
...
test("ProjectPanel: プロジェクト削除確認でDELETE APIが呼ばれる", async ({
  page,
}) => {
  let deleteApiCalled = false;
  await setupApiMocks(page);
...
test("ProjectPanel: プロジェクト削除後にドロップダウンからプロジェクトが消える", async ({
  page,
}) => {
  await setupApiMocks(page);
  await page.route(`**/api/projects/${MOCK_PROJECT.id}`, async (route) => {
  ...
  // Project name should no longer appear in the dropdown options
  const opts = await page.locator("select option").all();
  const names = await Promise.all(opts.map((o) => o.textContent()));
...
test("AlignmentPanel: IP点追加でPUT APIが呼ばれる", async ({ page }) => {
  ...
  // Override the PUT ip-points handler to track calls
  await page.route(
    `**/api/projects/${MOCK_PROJECT.id}/alignments/*/ip-points`,
    async (route) => {
  ...
  await page.getByRole("textbox", { name: "線形名" }).fill("API確認路線");
  await page.getByRole("button", { name: "線形を作成" }).click();
  await expect(
    page.getByRole("button", { name: "API確認路線", exact: true }),
  ).toBeVisible();

  await page.getByLabel("IP点X座標").fill("100");
  await page.getByLabel("IP点Z座標").fill("200");
  await page.getByLabel("曲線半径").fill("50");

  const responsePromise = page.waitForResponse(
    (r) => r.url().includes("/ip-points") && r.request().method() === "PUT",
  );
  await page.getByRole("button", { name: "IP 点を追加" }).click();
  await responsePromise;

  expect(putCalled).toBe(true);
});

test("AlignmentPanel: 線形削除でDELETE APIが呼ばれる", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "線形名" }).fill("削除API確認路線");
  await page.getByRole("button", { name: "線形を作成" }).click();
  await expect(
    page.getByRole("button", { name: "削除API確認路線", exact: true }),
  ).toBeVisible();

  const responsePromise = page.waitForResponse(
    (r) => r.url().includes("/alignments/") && r.request().method() === "DELETE",
  );
  await page.getByLabel("削除API確認路線を削除").click();
  await responsePromise;

  expect(deleteCalled).toBe(true);
});

test("AlignmentPanel: 縦断線形削除でDELETE APIが呼ばれる", async ({ page }) => {
  let verticalDeleteCalled = false;
  await setupAlignmentTests(page);
  ...
  await page.getByRole("textbox", { name: "線形名" }).fill("縦断削除確認");
  await page.getByRole("button", { name: "線形を作成" }).click();
  await page.getByRole("button", { name: "縦断線形タブ" }).click();
  await page.getByRole("textbox", { name: "縦断線形名" }).fill("縦断削除A");
  await page.getByRole("button", { name: "作成" }).click();
  await expect(page.getByRole("button", { name: "VIP を追加" })).toBeVisible();

  const responsePromise = page.waitForResponse(
    (r) =>
      r.url().includes("/verticals/") && r.request().method() === "DELETE",
  );
  await page.getByLabel("縦断削除Aを削除").click();
  await responsePromise;

  expect(verticalDeleteCalled).toBe(true);
});

// ---- Error state tests -------------------------------------------------------

test("ProjectPanel: プロジェクト一覧取得エラー時にエラーメッセージが表示される", async ({
  page,
}) => {
  await page.route("**/api/auth/login", async (route) => {
    await route.fulfill({
...
test("ログイン: メールアドレス入力欄はrequired属性を持つ", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: "ログイン" }).click();
  ...
  // The email input should have the required attribute
...
test("viewport: 各種キー操作でJS例外が発生しない", async ({
  page,
}) => {
  const jsErrors: string[] = [];
  page.on("pageerror", (err) => {
...
test("SettingsPanel: グリッドサイズスライダーをドラッグするとJS例外が発生しない", async ({
  page,
}) => {
  const jsErrors: string[] = [];
  page.on("pageerror", (err) => {
  ...
  await page.goto("/");
  await page.getByRole("button", { name: "設定" }).click();
  const slider = page.getByRole("slider", { name: "グリッドサイズ" }).first();
...
test("LayerPanel: レイヤー名が空の場合に追加ボタンが無効化される", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: "レイヤー" }).click();
...
test("LayerPanel: レイヤー追加後に入力欄がクリアされる", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "レイヤー" }).click();
  await page.getByPlaceholder("新しいレイヤー名").fill("クリアテスト");
  await page.getByRole("button", { name: "+" }).click();
  await expect(page.getByPlaceholder("新しいレイヤー名")).toHaveValue("");
});
...
test("GIS: 背景地図スタイルボタンが複数表示される", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "GIS" }).click();
  // Basemap option buttons should be visible
  await expect(page.getByRole("button", { name: "OpenStreetMap" })).toBeVisible();
  await expect(
    page.getByRole("button", { name: "地理院地図（標準）" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "地理院地図（淡色）" }),
  ).toBeVisible();
});

// ---- Header extended --------------------------------------------------------

test("Header: ArcSphere3D タイトルとバージョンバッジが表示される", async ({
  page,
}) => {
  await page.goto("/");
  await expect(page.getByText("ArcSphere3D")).toBeVisible();
...
test("Header: ログイン前はログアウトボタンが表示されない", async ({ page }) => {
  await page.goto("/");
