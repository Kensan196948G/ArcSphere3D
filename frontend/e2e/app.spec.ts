...
async function setupAlignmentTests(page: Page) {
  ...
  await page.route(`**/api/projects/${MOCK_PROJECT.id}/alignments`, async (route) => {
    if (route.request().method() === "GET") {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
      ...
      const body = (await route.request().postDataJSON()) as { name?: string; design_speed?: number };
      await route.fulfill({
        status: 201,
        contentType: "application/json",
        body: JSON.stringify({
...
test("LeftMenu: BIM パネルが表示されIFCガイドメッセージが見える", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.getByText("IFC ファイルを読み込むと BIM 属性パネルが有効になります")).toBeVisible();
});

test("LeftMenu: GIS パネルが表示され背景地図セレクターが見える", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "GIS" }).click();
  // Heading text is "GIS 背景地図"
  await expect(page.getByRole("heading", { name: "GIS 背景地図" })).toBeVisible();
...
test("LeftMenu: CADパネルに切り替わると実装予定メッセージが表示される", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "CAD" }).click();
  await expect(page.getByText("STEP / CAD 読み込み — 近日実装予定")).toBeVisible();
});

test("AlignmentPanel: プロジェクト未選択時に案内メッセージが表示される", async ({ page }) => {
  await setupApiMocks(page);
  await page.goto("/");
  ...
  await expect(page.getByRole("button", { name: "ログアウト" })).toBeVisible();
  await page.getByRole("button", { name: "線形" }).click();
  await expect(page.getByText("プロジェクトを選択すると線形一覧が表示されます。")).toBeVisible();
});

test("ProjectPanel: 新プロジェクト名を入力して作成ボタンを押せる", async ({ page }) => {
  const NEW_PROJECT = {
...
test("ProjectPanel: プロジェクト選択後にファイルアップロードボタンが表示される", async ({ page }) => {
  await setupApiMocks(page);
  await page.goto("/");
  await page.getByRole("button", { name: "ログイン" }).click();
  await page.getByLabel("パスワード").fill("arcsphere-demo");
...
test("ProjectPanel: ファイル一覧に3Dボタンと削除ボタンが表示される", async ({ page }) => {
  await setupApiMocks(page);
  await page.goto("/");
  await page.getByRole("button", { name: "ログイン" }).click();
  await page.getByLabel("パスワード").fill("arcsphere-demo");
...
test("AlignmentPanel: IP点フォームにX・Z・R座標を入力できる", async ({ page }) => {
  await setupAlignmentTests(page);
  await page.getByRole("textbox", { name: "線形名" }).fill("Route A");
  await page.getByRole("button", { name: "線形を作成" }).click();
  await expect(page.getByRole("button", { name: "Route A", exact: true })).toBeVisible();
  // IP点フォームが表示されていること
  await expect(page.getByRole("button", { name: "IP 点を追加" })).toBeVisible();
  // 座標入力フォームに値を入力できる
...
test("AlignmentPanel: 線形を削除するとリストから消える", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "線形を作成" }).click();
  await expect(page.getByRole("button", { name: "削除テスト路線", exact: true })).toBeVisible();
  // 削除ボタンをクリック
  await page.getByLabel("削除テスト路線を削除").click();
  // リストから消える
  await expect(page.getByRole("button", { name: "削除テスト路線", exact: true })).not.toBeVisible();
});
