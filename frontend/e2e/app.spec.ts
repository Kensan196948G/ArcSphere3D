...
test("ProjectPanel: ファイル削除ボタンクリックでファイルが一覧から消える", async ({
  page,
}) => {
  let fileDeleted = false;
  await setupApiMocks(page);
  // Override files endpoint to return empty list after deletion
  await page.route(
    `**/api/projects/${MOCK_PROJECT.id}/files/${MOCK_FILE.id}`,
    async (route) => {
      if (route.request().method() === "DELETE") {
        fileDeleted = true;
        await route.fulfill({ status: 204 });
  ...
  // After deletion the file list should return empty
  let callCount = 0;
  await page.route(
    `**/api/projects/${MOCK_PROJECT.id}/files`,
    async (route) => {
      callCount++;
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(callCount === 1 ? [MOCK_FILE] : []),
      });
    },
  );
  ...
  await page.selectOption("select", MOCK_PROJECT.id);
  await expect(page.getByText("cube.stl")).toBeVisible();
  // Click delete button for the file
...
test("SettingsPanel: 環境光スライダーが表示される", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "設定" }).click();
  // Ambient light label is visible
  await expect(page.getByText(/環境光/)).toBeVisible();
});
...
test("AlignmentPanel: 線形名が空の場合は作成ボタンが無効化される", async ({
  page,
}) => {
  await setupAlignmentTests(page);
  // Clear the name field
  const nameInput = page.getByRole("textbox", { name: "線形名" });
