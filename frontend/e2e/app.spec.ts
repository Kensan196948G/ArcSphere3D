...
test("BottomConsole: 初期状態の準備完了メッセージが表示される", async ({
  page,
}) => {
  await page.goto("/");
  await expect(
    page.getByText("ArcSphere3D 準備完了。右パネルからモデルを読み込むか、ドラッグ＆ドロップしてください。"),
  ).toBeVisible();
});
...
test("ModelPanel: ファイル読み込みセクションの説明テキストが表示される", async ({
  ...
  await page.getByRole("button", { name: "モデル" }).click();
  await expect(
    page.getByText("ローカルファイルをブラウザだけで解析します。アップロードは行いません。"),
  ).toBeVisible();
});
...
test("ProjectPanel: 3Dビューアで開くボタンのクリックでダウンロードURL APIが呼ばれる", async ({
  ...
  const responsePromise = page.waitForResponse(`**/api/files/${MOCK_PROJECT.id}/${MOCK_FILE.id}/download`);
